...
for page in range(1, 3):
    params = {
        ...
        "platform": "1",
        "partner": "126",
        "originUrl": f"https://bj.meituan.com/meishi/pn{page}",
        "riskLevel": "1",
        "optimusCode": "10",
        "_token": token
    }
    ...
    data = response.json()
    # pprint.pprint(data)
    poiInfos = data['data']['poiInfos']
    for poiInfo in poiInfos:
        # pprint.pprint(poiInfo)
        title = poiInfo['title']    # 标题
        poiId = poiInfo['poiId']    # 店铺ID
        avgScore = poiInfo['avgScore']    # 评分
        allCommentNum = poiInfo['allCommentNum']    # 总评论量
        avgPrice = poiInfo['avgPrice']    # 平均价格
        detail_url = f'https://bj.meituan.com/meishi/{poiId}/'
        print(title, avgPrice, detail_url)
        # break
    print("-----" * 30)
    break
