...
let encode_file = "C:\\Y\\Case\\working\\猿人学\\in.js"
let decode_file = "C:\\Y\\Case\\working\\猿人学\\out.js"


function get_local_sojson(a = encode_file, b = decode_file) {
    // 读取需要解码的js文件, 注意文件编码为utf-8格式
    let jscode = fs.readFileSync(a, {encoding: "utf-8"});
    result = muti_process_sojsondefusion(jscode);
    console.log(result);
    // fs.writeFile(b, code, (err) => {});
    return result;
}

// get_local_sojson(a, b);


// 方法二：
function get_sojson(jscode) {
    result = muti_process_sojsondefusion(jscode);
    console.log(result);
    // fs.writeFile(b, code, (err) => {});
    return result;
