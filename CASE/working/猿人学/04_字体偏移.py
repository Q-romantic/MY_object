...
""" 给定数字图片显与不显控制，css偏移排序数字（规律：总偏移量总和为零，且单个偏移量必是11.5的整倍数） """
...
for i in range(1, 6):
    ...
    kv = key + value
    b64_kv = base64.encodebytes(bytes(kv.encode('utf-8'))).decode('utf-8')
    b64_kv = b64_kv[:-2]
    hex_md5 = hashlib.md5(b64_kv.encode())
    x = hex_md5.hexdigest()
    data = response.json()["info"]
    obj = re.compile('<td>(?P<td>.*?)</td>', re.S)
    tds = obj.findall(data)
    for j in tds:
        obj = re.compile('src="(?P<a>.*?)" class="img_number (?P<b>.*?)" style="left:(?P<c>.*?)px">', re.S)
        a = obj.findall(j)
        s_list = []
        for k in a:
            if x in k:  # 排除掉不显示数字（通过key，value加密后去掉最后一位=号）
                pass
            else:
                s_list.append([num_dict[k[0]], k[2]])
        new_num = s_list.copy()  # css偏移给获取数字排序
        for k, tup in enumerate(s_list):
            index = int(float(tup[1]) / 11.5)
            if index != 0:
                new_num[k + index] = s_list[k]
        num_str = ''.join([str(n[0]) for n in new_num])
        total.append(int(num_str))
        print(s_list, '\t>>css偏移>>\t', new_num)
    print('---' * 30, f'第{i}页数据')
print(total)
print(len(total))
answer = sum(total)
...
2077, 2001, 5863, 1506, 1164, 4131, 9512, 1942, 5473, 6382, 
8488, 3105, 3295, 5238, 7209, 2786, 7688, 9769, 374, 2679, 
6462, 6200, 712, 7586, 4230, 9837, 90, 8684, 6479, 9949,
5902, 2026, 8295, 2791, 1634, 2419, 4697, 2731, 1680, 2313]
"""